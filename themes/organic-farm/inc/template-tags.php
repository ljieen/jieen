...
function organic_farm_entry_footer() {
	$separate_meta = __( ', ', 'organic-farm' );
	$categories_list = get_the_category_list( $separate_meta );
	$tags_list = get_the_tag_list( '', $separate_meta );
	if ( ( ( organic_farm_categorized_blog() && $categories_list ) || $tags_list ) || get_edit_post_link() ) {
...
function organic_farm_categorized_blog() {
	$category_count = get_transient( 'organic_farm_categories' );
	...
	if ( false === $category_count ) {
		...
		set_transient( 'organic_farm_categories', $category_count );
	}
