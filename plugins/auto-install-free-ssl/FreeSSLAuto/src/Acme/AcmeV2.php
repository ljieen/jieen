...
use AutoInstallFreeSSL\FreeSSLAuto\DnsApi\DnsApi;
                ...
                if ('http-01' === $this->challenge) {
                    ...
                    if (!file_put_contents($tokenPath, $payload)) {
                        ...
                        if (!file_put_contents($tokenPathAlternative, $payload)) {
                            $this->logger->log("Sorry, token for ${domain} was NOT SAVED neither at regular path ${tokenPath} nor Alternative Path ${tokenPathAlternative} due to some issue. Please make a directory '.well-known' (with permission 0755) in ".$this->webRootDir.' and try again.');
                            continue;
                        } else {
                            $this->logger->log("Token for ${domain} successfully saved at the Alternative Path ${tokenPathAlternative}");
                            chmod($tokenPathAlternative, 0644);
                            
                            if($this->factory->fix_htaccess_document_root($web_root_dir)){
                                $this->logger->log("htaccess rules have been created successfully in the Document root directory $web_root_dir . Now the challenge token should be available at ${uri}");
                            }
                            else{
                                $this->logger->log("Oops! Attempt to create htaccess rules in the Document root directory has been failed: $web_root_dir");
                            }
                     ...
                     *  */
                     ...
                     if (!$this->factory->verify_internally_http_wp($payload, $uri)) {
                         ...
                         $this->logger->log("1st Internal Validation: Payload content does not match with content of the challenge URI.");
                            ...
                            if($this->factory->fix_htaccess_challenge_dir($web_root_dir.DS.'.well-known')){
                                ...
                                $this->logger->log("htaccess rules have been created successfully in the '.well-known' directory.");
                            }
                            else{
                                
                                $this->logger->log("Oops! There was an error creating htaccess rules in the '.well-known' directory.");                                
                            }
                            
                            if (!$this->factory->verify_internally_http_wp($payload, $uri)) {
                                
                                $this->logger->log("2nd Internal Validation: Payload content does not match with content of the challenge URI.");
                            
                                    $tokenPathAlternative = $web_root_dir.DS.$challenge['token'];
                                    
                                    if (file_put_contents($tokenPathAlternative, $payload) !== false){
                                        
                                        $this->logger->log("Token for ${domain} successfully saved at the Alternative Path ${tokenPathAlternative}");
                                    
                                        //Create htaccess rules in Document root directory to fix the issue                                    
                                        if($this->factory->fix_htaccess_document_root($web_root_dir)){
                                            $this->logger->log("htaccess rules have been created successfully in the Document root directory $web_root_dir . Now the challenge token should be available at ${uri}");
                                        }
                                        else{
                                            $this->logger->log("Oops! Another attempt to create htaccess rules in the Document root directory has been failed: $web_root_dir");
                                        }
                                        
                                        if (!$this->factory->verify_internally_http_wp($payload, $uri)) {
                                                                                        
                                            $this->logger->log("3rd and Final Internal Validation: Payload content (${payload}) does not match the content of ${uri}. Either ${domain} is not pointed to this server or unavailable over HTTP due to some server-side issue. Please fix this issue and try again.");
                                     ...
                                     $this->logger->log("Token for ${domain} want NOT saved at the Alternative Path ${tokenPathAlternative} due to an error.");
                                     $this->logger->log("Payload content (${payload}) does not match the content of ${uri}. Either ${domain} is not pointed to this server or unavailable over HTTP due to some server-side issue. Please fix this issue and try again.");
                    ...
                    $this->logger->log("Final Internal Validation: the Payload content (${payload}) successfully matched with the content of ${uri}.");
